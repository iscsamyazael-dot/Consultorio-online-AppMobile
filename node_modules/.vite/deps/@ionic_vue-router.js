import { Yn as shallowRef } from "./vue.runtime.esm-bundler-J3hhHSON.js";
import { i as createRouter$1, o as createWebHashHistory$1, r as createMemoryHistory$1, s as createWebHistory$1, v as parseQuery } from "./vue-router-OaAEK9Ox.js";
		...
		*/
		...
		if (routeInfo) {
			...
			*/
			...
			if (routeInfo.pathname === pathname) router.push(Object.assign({ path: routeInfo.pathname }, push));
			else router.push(Object.assign({ path: pathname }, push));
		} else handleNavigate(pathname + hrefSearch + hrefHash, "push", "none", void 0, tab);
	};
	/**
...
var createViewStacks = (router) => {
	...
	*/
	const mountIntermediaryViews = (outletId, viewItem, delta = 1) => {
		...
		if (startIndex === -1) return;
		const endIndex = Math.min(viewStack.length, startIndex + delta);
		for (let i = startIndex + 1; i < endIndex; i++) viewStack[i].mount = true;
	};
	return {
		unmountLeavingViews,
